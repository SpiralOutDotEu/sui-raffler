...
/// Test that non-admin cannot claim protocol fees
#[test]
#[expected_failure(abort_code = sui_raffler::ENotAdmin)]
fun test_claim_protocol_fees_unauthorized() {
    let admin = @0xAD;
    let non_admin = @0xBEEF;
    let controller = @0x1235;
    let fee_collector = @0xFEE5;
    ...
    // Release raffle as controller
    ts.next_tx(controller);
    clock.set_for_testing(1001);
    sui_raffler::release_raffle(&config, &mut raffle, &random_state, &clock, ts.ctx());

    // Try to claim protocol fees as non-admin
    ts.next_tx(non_admin);
...
/// Test that cannot claim protocol fees twice
#[test]
#[expected_failure(abort_code = sui_raffler::EAlreadyClaimed)]
fun test_claim_protocol_fees_twice() {
    let admin = @0xAD;
    let controller = @0x1235;
    let fee_collector = @0xFEE5;
    let organizer = @0x1234;
    ...
    // Try to claim protocol fees again
    ts.next_tx(admin);
    sui_raffler::claim_protocol_fees(&config, &mut raffle, ts.ctx());
